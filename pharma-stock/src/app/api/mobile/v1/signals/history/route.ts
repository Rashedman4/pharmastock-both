...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [countResult, dataResult] = await Promise.all([
      pool.query('SELECT COUNT(*)::int AS total FROM signal_history'),
      pool.query(
        `SELECT id, symbol, entrance_date, closing_date, in_price, out_price, success, created_at
         FROM signal_history
    ...
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch signal history' } },
      { status: 500 }
    );
  }
}
