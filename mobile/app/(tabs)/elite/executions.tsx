...
import { executionStatusLabel } from '@/lib/eliteStatus';
    ...
    <Card>
      <View style={styles.cardHeader}>
        ...
        <View style={styles.badges}>
          ...
          />
          <Badge label={executionStatusLabel(item.status, t)} variant={execStatusVariant(item.status)} />
        </View>
      </View>

      <View style={styles.row}>
        <View style={styles.col}>
          <Text style={styles.colLabel}>{t('elite.executed_price')}</Text>
          <Text style={styles.colValue}>${Number(item.executed_price).toFixed(4)}</Text>
        </View>
        <View style={styles.col}>
          <Text style={styles.colLabel}>{t('elite.quantity')}</Text>
          <Text style={styles.colValue}>{formatNumber(item.executed_quantity)}</Text>
        </View>
        <View style={styles.col}>
          <Text style={styles.colLabel}>{t('elite.date_label')}</Text>
